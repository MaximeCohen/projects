...
void loop()                     // run over and over again
{
  ...
  if (millis() - timer > 2000) { 
    ...
    int heure = (GPS.hour, DEC) + 3;
    int lat_ent = GPS.latitude/100;
    float latitude = lat_ent + ((GPS.latitude - (lat_ent * 100))/60);
    int long_ent = GPS.longitude/100;
    float longitude = long_ent + ((GPS.longitude - (long_ent * 100))/60);
